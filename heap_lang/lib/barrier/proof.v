...
Proof.
  ...
  iApply ("HΦ" with "==>[-]").
  iShift (saved_prop_alloc (F:=idCF) P) as (γ) "#?".
  iShift (sts_alloc (barrier_inv l P) _ N (State Low {[ γ ]}) with "[-]")
    as (γ') "[#? Hγ']"; eauto.
  ...
  { iApply sts_ownS_op; eauto using i_states_closed, low_states_closed.
    ...
    - iApply (sts_own_weaken with "Hγ'");
        ...
        abstract set_solver. }
  iShiftIntro. rewrite /recv /send. iSplitL "Hr".
...
Proof.
  ...
  iShift (sts_openS (barrier_inv l P) _ _ γ with "[Hγ]")
    as ([p I]) "(% & [Hl Hr] & Hclose)"; eauto.
  destruct p; [|done]. wp_store. iFrame "HΦ".
  iShift ("Hclose" $! (State High I) (∅ : set token) with "[-]"); last done.
...
Proof.
  ...
  iShift (sts_openS (barrier_inv l P) _ _ γ with "[Hγ]")
    as ([p I]) "(% & [Hl Hr] & Hclose)"; eauto.
  wp_load. destruct p.
  - iShift ("Hclose" $! (State Low I) {[ Change i ]} with "[Hl Hr]") as "Hγ".
    ...
    iShiftIntro. wp_op=> ?; simplify_eq; wp_if.
  ...
  - (* a High state: the comparison succeeds, and we perform a transition and
    ...
    iShift ("Hclose" $! (State High (I ∖ {[ i ]})) (∅ : set token) with "[HΨ' Hl Hsp]").
    ...
    iShiftIntro. wp_op=> ?; simplify_eq/=; wp_if.
    iShiftIntro. iApply "HΦ". iApply "HQR". by iRewrite "Heq".
Qed.
...
Proof.
  ...
  iShift (sts_openS (barrier_inv l P) _ _ γ with "[Hγ]")
    as ([p I]) "(% & [Hl Hr] & Hclose)"; eauto.
  iShift (saved_prop_alloc_strong (R1: ∙%CF (iProp Σ)) I) as (i1) "[% #Hi1]".
  iShift (saved_prop_alloc_strong (R2: ∙%CF (iProp Σ)) (I ∪ {[i1]}))
    as (i2) "[Hi2' #Hi2]"; iDestruct "Hi2'" as %Hi2.
  rewrite ->not_elem_of_union, elem_of_singleton in Hi2; destruct Hi2.
  iShift ("Hclose" $! (State p ({[i1; i2]} ∪ I ∖ {[i]}))
  ...
  { iApply sts_ownS_op; eauto using i_states_closed, low_states_closed.
    ...
    - iApply (sts_own_weaken with "Hγ");
      ...
      abstract set_solver. }
  iShiftIntro; iSplitL "Hγ1"; rewrite /recv /barrier_ctx.
